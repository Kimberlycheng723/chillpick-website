...
const getRecommendations = async (userId) => {
  try {
    console.log('Recommendation request for:', userId ? `user ${userId}` : 'anonymous');
...
const getDefaultRecommendations = async () => {
  try {
    ...
    if (movies.length < movieCount || books.length < bookCount) {
      console.warn('⚠️ Not enough movies or books fetched to meet the requirement.');
      return [];
    }
    ...
    const shuffledMovies = shuffleArray(movies).slice(0, movieCount).map(rec => normalize(rec, 'Movie'));
    const shuffledBooks = shuffleArray(books).slice(0, bookCount).map(rec => normalize(rec, 'Book'));
    const combined = shuffleArray([...shuffledMovies, ...shuffledBooks]);
    console.log(`✅ Final list: ${combined.length} items (Movies: ${shuffledMovies.length}, Books: ${shuffledBooks.length})`);
    return combined;
  } catch (error) {
...
const getPersonalizedRecommendations = async (userId) => {
  try {
    ...
    if (movies.length >= 4) {
      ...
      console.log(`🎲 Randomly selected 4 movies from ${movies.length} available`);
    } else {
      // Use all available movies and fill with trending movies if needed
      selectedMovies = movies;
      if (selectedMovies.length < 4) {
        console.log(`⚠️ Only ${selectedMovies.length} personalized movies, fetching trending to fill gap`);
        const fallbackMovies = await tmdbService.getTrendingMovies(4 - selectedMovies.length);
        const normalizedFallback = fallbackMovies.slice(0, 4 - selectedMovies.length)
    ...
    if (books.length >= 4) {
      ...
      console.log(`🎲 Randomly selected 4 books from ${books.length} available`);
    } else {
      // Use all available books and fill with popular books if needed
      selectedBooks = books;
      if (selectedBooks.length < 4) {
        console.log(`⚠️ Only ${selectedBooks.length} personalized books, fetching popular to fill gap`);
        const fallbackBooks = await googleBooksService.getPopularBooks(4 - selectedBooks.length);
        const normalizedFallback = fallbackBooks.slice(0, 4 - selectedBooks.length)
    ...
    console.log(`✅ Personalized recommendations: ${final.length} items (${selectedMovies.length} movies, ${selectedBooks.length} books)`);
    console.log(`🎲 User will see different results on each refresh due to random selection`);
    
    return final;

  // // Select top items
  //   let topMovies = movies.slice(0, 4);
  //   let topBooks = books.slice(0, 4);
    
  //   // Fallback if not enough items
  //   if (topMovies.length < 4) {
  //     const fallbackMovies = await tmdbService.getTrendingMovies(4 - topMovies.length);
  //     topMovies = [...topMovies, ...fallbackMovies.slice(0, 4 - topMovies.length)];
  //   }
    
  //   if (topBooks.length < 4) {
  //     const fallbackBooks = await googleBooksService.getPopularBooks(4 - topBooks.length);
  //     topBooks = [...topBooks, ...fallbackBooks.slice(0, 4 - topBooks.length)];
  //   }

  //   // Combine and shuffle
  //   const final = shuffleArray([...topMovies, ...topBooks]);
  //   console.log(`✅ Personalized recommendations: ${final.length} items`);
  //   return final;
...
// CHANGED: Simplified to only track genres from clicked items, watchlist, and filters
const analyzeUserPreferences = (interactions) => {
const genreOccurrences = {
    genres: {}, // Will store genre names and their counts
    topGenres: [] // Will store top genres by count
  };
  
  interactions.forEach((interaction) => {
    // Track clicked items and their genres
    if (interaction.interactionType === 'item clicked' && interaction.clickedItem) {
      const itemGenres = interaction.clickedItem.genres || interaction.clickedItem.genre || [];
      
      if (Array.isArray(itemGenres)) {
        itemGenres.forEach(genre => {
          genreOccurrences.genres[genre] = (genreOccurrences.genres[genre] || 0) + 1;
        });
      }
    }
    
    // Track watchlisted items and their genres
    if (interaction.interactionType === 'add to watchlist' && interaction.itemDetails) {
      const itemGenres = interaction.itemDetails.genres || interaction.itemDetails.genre || [];
    ...
    if (interaction.interactionType === 'filter genre' && interaction.genre) {
      genreOccurrences.genres[interaction.genre] = (genreOccurrences.genres[interaction.genre] || 0) + 1;
    }
  });
...
const refreshRecommendations = async (targetCount = 8) => {
  try {
    ...
    console.log(`📊 Final distribution: ${movieCount} movies, ${bookCount} books`);
...
// const refreshRecommendations = async (userId = null) => {
//   try {
//     console.log(`🔄 Refreshing recommendations for ${userId ? `user ${userId}` : 'anonymous user'}...`);

//     // Reuse the same logic as getRecommendations
...
module.exports = {
  ...
  analyzeUserPreferences // Export this for the debug route
};
